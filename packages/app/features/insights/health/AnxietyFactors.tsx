...
export const AnxietyFactors: React.FC<AnxietyFactorsProps> = ({ date }) => {
  ...
  const { loading: loadingAuthentication, authenticated, authenticate } = useSahhaHook()
  ...
  const uri = 'https://sandbox.webview.sahha.ai/app' // +
  // (date ? `?date=${moment(date).format('YYYY-MM-DD')}` : '')

  return (
    <YStack f={1}>
      <XStack>
        <Button onPress={authenticate}>Re-Authenticate</Button>
      </XStack>
      <WebView
        source={{
          uri,
